...
//   - payload 与返回值都只是有效数据本身，默认原样透传，库不包任何信封。
//     唯一例外是 lambda tunnel 模式（lambda://<fn>/<path>）：对端是 lambda 框架
//     reqresp 模式的函数时，路径必须 in-band，库代为包/拆 reqresp 传输信封
//     ——信封内仍是裸业务数据（service 应用信封由对端引擎自行包拆，客户端不感知）。
//   - 一切失败（传输失败、HTTP 非 2xx、Lambda 函数内部错误、tunnel in-band 错误）
//     都通过 err 表达，不返回多余信息。
//
...
const (
	...
	backendLambda backend = "lambda" // lambda://<fn>[/<path>]，lambda.Invoke
...
// tunnel in-band 错误、tunnel 响应非信封 ErrBadResponse），此时 body 恒为 nil；
// err 为 nil 时 body 是后端返回的有效数据（HTTP 响应体 / Lambda 返回 payload；
// tunnel 模式下为拆完信封的业务数据）。
